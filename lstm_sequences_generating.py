...
class LSTMWrapper:
    def __init__(self,embedding_dim, hidden_dim, epoches):
    ...
    def train(self):
        ...
        for epoch in range(self.epoches):  # again, normally you would NOT do 300 epochs, it is toy data
        ...
        result_line = "embedim_dim {}, hidden_dim {}, epoches {},  Accuracy: {}\n".format(self.embedding_dim, self.hidden_dim,self.epoches, accuracy)
        output_line = "{},{},{},{}\n".format(self.embedding_dim, self.hidden_dim,self.epoches, accuracy)
        print(result_line)

        lstm_test_result = "./tpcds_lstm_test_result.csv"
        # sql_file = os.path.join(result_dir, sql_file_name)
        with open(lstm_test_result, 'a') as file:
...
if __name__ == '__main__':
    ...
    hidden_dim = 6
    epoches= 500
    for emb_i in range(3, 15,3):
        for hi_i in range(5,20,2):
            for epo_i in range (300, 1100, 200):
                lstm_wrapper = LSTMWrapper(embedding_dim=emb_i, hidden_dim=hi_i, epoches=epo_i)
                lstm_wrapper.train()
    # lstm_wrapper = LSTMWrapper(embedding_dim=embedim_dim, hidden_dim=hidden_dim,epoches=epoches)
    # lstm_wrapper.train()
